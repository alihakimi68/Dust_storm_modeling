...
Residual_test_accuracy = []
Residual_test_precission = []
GWML_test_accuracy = []
GWML_test_precission = []

Residual_val_accuracy = []
Residual_val_precission = []
GWML_val_accuracy = []
GWML_val_precission = []

bandwidth = [x for x in range(50, 300)]
...
for bw in bandwidth:
    ...
    print(f'############# The size of the test dataset{bw} ##############')
...
axes[0].plot(bandwidth, GWML_val_accuracy, label='GWML Validation Accuracy')
axes[0].plot(bandwidth, GWML_test_accuracy, label='GWML test Accuracy')
axes[0].set_xlabel('Bandwidth size')
axes[0].set_ylabel('Accuracy')
axes[0].set_title('GWML Validation/Test')
axes[0].legend()
axes[0].grid(True)

# Plot 4: GWML Test
axes[1].plot(bandwidth, GWML_val_precission, label='GWML Validation Precision')
axes[1].plot(bandwidth, GWML_test_precission, label='GWML Test Precision')
axes[1].set_xlabel('Bandwidth size')
axes[1].set_ylabel('Precision')
axes[1].set_title('GWML Validation/Test')
axes[1].legend()
axes[1].grid(True)

plt.tight_layout()
# plt.show()
plt.savefig('Test_Bandwidth_change_RandomForest.jpg')
plt.close()