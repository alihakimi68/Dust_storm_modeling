...
data_folder = "D:/University/DustStorming/ToAli/DustStormModeling/R/"
df = pd.read_csv(data_folder + "Landslide_label_new.csv")
...
coords = train_valid_df[['X_utm','Y_utm']]

# Drop the specified coordinates from the train data frame
train_valid_df = train_valid_df.drop(columns=['X_utm','Y_utm'])

# Use train_test_split to create indices for random sampling
X = train_valid_df.drop(['Target','FID'], axis=1)
y = train_valid_df['Target']
...
def bootstrapWeighted(X_train, y_train, case_weights):
    ...
    # Calculate the number of samples to be included in the bootstrap sample (63%)
    num_bootstrap_samples = int(0.63 * num_samples)

    # Select the first 63% of indices without replacement based on case weights
    bootstrap_indices = np.random.choice(num_samples, num_bootstrap_samples, replace=True,
                                         p=case_weights / np.sum(case_weights))

    # The remaining 36% of indices are for OOB
...
for m in range(0,obs):
    ...
    if kernel == 'adaptive':
        ...
        while len(SubSet['Target'].unique()) < 2 or any(SubSet['Target'].value_counts() < mincatobs):
        ...
        if cc > 1 :
            ...
            value_counts = SubSet['Target'].value_counts()
            ...
            max_category_indices = SubSet[SubSet['Target'] == max_category].index
    ...
    train_indices = np.random.choice(len(SubSet['Target']), size=int(0.8 * len(SubSet['Target'])), replace=False)
    ...
    X_train_l = SubSet_train.drop(['Target'], axis=1)
    X_test_l = SubSet_valid.drop(['Target'], axis=1)
    y_train_l = SubSet_train['Target']
    y_test_l = SubSet_valid['Target']
    ...
    while len(y_train_l.unique()) < 2:
        ...
        train_indices = np.random.choice(len(SubSet['Target']), size=int(0.8 * len(SubSet['Target'])),
                                         replace=False)
    ...
    prediction_row = pd.DataFrame({'PointID': X_test_l['pointID'],
                                   ...
                                   'FalsePositive': 0,
                                   'FalseNegative': 0}, )
    prediction_rows_list.append(prediction_row)
    ...
    validation_OOB_row = pd.DataFrame({'PointID': X_OOB['pointID'],
                                   ...
                                   'FalsePositive': 0,
                                   'FalseNegative': 0}, )
    validation_OOB_row_list.append(validation_OOB_row)

    prediction_row_2nd = pd.DataFrame({'PointID': X_test_l['pointID'],
                                   'y_test_l': y_test_l,
                                   'y_pred_l': y_pred_l,
                                   'TruePositive': 0,
                                   'TrueNegative': 0,
...
prediction_df = pd.concat(prediction_rows_list, ignore_index=True)
Validation_OOB_df = pd.concat(validation_OOB_row_list, ignore_index=True)
...
actual_values_2nd = prediction_df_2nd['y_test_l']
predicted_values_2nd = prediction_df_2nd['y_pred_l']
