...
columns_to_convert <- c(2, 13, 17:25) # df_dustsources_WS0_X_0_PN20_SP_
...
train_valid.df <- df
## Local Random Forest
Coords <- train_valid.df[, c('Y','X')]

# Drop the specified coordinates from the train data frame
train_valid.df <- train_valid.df[, !(colnames(train_valid.df) %in% c('X','Y'))]
