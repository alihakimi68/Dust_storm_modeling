grf.bw <- function(formula, dataset, kernel = "adaptive", coords, bw.min = NULL, bw.max = NULL, step = 1, trees = 500, mtry = NULL, importance = "impurity", nthreads = 1, forests = FALSE, geo.weighted = TRUE, ...) {
  ...
  if (is.null(bw.min)) {
    bw.min <- max(round(Obs * 0.05, 0), ModelVarNo + 2, 20)
  }
  if (is.null(bw.max)) {
    bw.max <- max(round(Obs * 0.95, 0), ModelVarNo + 2)
  }
  if (is.null(mtry)) {
    mtry = max(floor(ModelVarNo / 3), 2)
  }
  ...
  for (abw in seq(from = bw.min, to = bw.max, by = step)) {
    ...
    # Update: Use metrics appropriate for binary classification
    metrics_bw_grf <- confusionMatrix(grf16.a$Global.Model$predictions, Y)
    eval.bw.grf[count, 2] <- metrics_bw_grf$overall["Accuracy"]

    message("Bandwidth: ", abw)
    message("Accuracy of Local Model: ", eval.bw.grf[count, 2])

    # Update: Use metrics appropriate for binary classification
    metrics_bw_grf <- confusionMatrix((grf16.a$LGofFit$LM_yfitOOB + grf16.a$Global.Model$predictions) / 2, Y)
    eval.bw.grf[count, 3] <- metrics_bw_grf$overall["Accuracy"]

    # Update: Use metrics appropriate for binary classification
    metrics_bw_grf <- confusionMatrix((grf16.a$LGofFit$LM_yfitOOB * 0.25) + (grf16.a$Global.Model$predictions * 0.75), Y)
    eval.bw.grf[count, 4] <- metrics_bw_grf$overall["Accuracy"]
