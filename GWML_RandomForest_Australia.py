...
DatasetToAnalyze = 'Australia' # Main , Distance, WindowsWMe, WindowsMVEM, Australia
################### Import the dataset #####################

os.chdir("D:/University/DustStorming/ToAli/DustStormModeling/For training/")


if DatasetToAnalyze == 'Australia':
    dustsourcespickle = 'AustralianAlps'


# dataset = pd.read_parquet(f'{dustsourcespickle}.parquet')
Maindataset = pd.read_parquet(f'{dustsourcespickle}.parquet')
# Calculate the number of rows to drop (90% of total rows)
rows_to_drop = int(0.99 * len(Maindataset))

# Randomly select rows to drop
rows_to_keep = Maindataset.sample(n=rows_to_drop, random_state=42)

# Drop the selected rows
dataset = Maindataset.drop(rows_to_keep.index)
dataset = dataset.dropna()
dataset = dataset[dataset['PET'] != 9999]
dataset = dataset[dataset['season'] == 'Autumn']
dataset.reset_index(drop=True, inplace=True)
dataset.rename(columns={'x': 'X', 'y': 'Y'}, inplace=True)
print(f'Processing a dataset with shape of {dataset.shape}')
if DatasetToAnalyze == 'Australia':
    dataset = dataset.drop(columns=['year','season'])

# Assuming 'z_score' is the column you want to exclude
columns_to_scale = [col for col in dataset.columns if col not in ['z_score','X','Y']]
...
if Case == 'Classification':
    ...
    params = {
        ...
        'min_samples_leaf': 5,
        'bootstrap': True,
        'min_impurity_decrease': 0.00035992231203731964,
        'ccp_alpha': 0.00012741348976487156,
        'max_samples': 0.5,
...
def calculate_distances(coords,m):
    first_point = np.array(coords.iloc[m][['X','Y']])  # Convert the first row to a numpy array
    distances = []
    for _, row in coords.iterrows():
        point_array = np.array(row[['X','Y']])  # Convert each row to a numpy array
        distance = euclidean_distance(first_point, point_array)
...
def bootstrapWeighted(X_train,y_train,case_weights,InBagSamples,n_splits,Case):
...
if Case == 'Classification':
    ...
    prediction_df = pd.DataFrame(columns=['PointID', 'y_test_l', 'y_pred_l'])

    Validation_OOB_df = pd.DataFrame(columns=['PointID', 'y_OOB', 'y_validation_OOB'])
...
for m in range(0,obs):
    # Get the data
    DNeighbour = calculate_distances(coords, m)
    ...
    if Case == 'Classification':
        while len(SubSet['z_score'].unique()) < 2 or SubSet['z_score'].value_counts()[0] < mincatobs or SubSet['z_score'].value_counts()[1] < mincatobs:
        ...
        if cc > 1:
            ...
            value_counts = SubSet['z_score'].value_counts()
            ...
            # Ensure that the indices to remove contain rows with max_category
            max_category_indices = SubSet[SubSet['z_score'] == max_category].index
        ...
        # plt.scatter(after_removal_coords['X'], after_removal_coords['Y'], c=after_removal_coords['z_score'],
        #             cmap='coolwarm', s=50, alpha=0.7, label='z_score')
    ...
    X_l = SubSet.drop(['z_score'], axis=1)
    y_l = SubSet['z_score']
    if Case == 'Calssification':
    ...
    if Case == 'Classification':
        ...
        prediction_row = pd.DataFrame({'PointID': X_test_l['pointID'],
                                       'y_test_l': y_test_l,
                                       'y_pred_l': y_pred_l,}, )
        local_model_mse_l[SubSet.loc[SubSet['DNeighbour'] == 0, 'pointID'].values[0]] = mse_l
        local_model_mae_l[SubSet.loc[SubSet['DNeighbour'] == 0, 'pointID'].values[0]] = mae_l
        local_model_r2_l[SubSet.loc[SubSet['DNeighbour'] == 0, 'pointID'].values[0]] = r2_l
    ...
    if Case == 'Classification':
        ...
        mse_OOB = mean_squared_error(y_OOB, y_validation_OOB)
        # Calculate MAE
        mae_OOB = mean_absolute_error(y_OOB, y_validation_OOB)
        # Calculate R2
        r2_OOB = r2_score(y_OOB, y_validation_OOB)
        validation_OOB_row = pd.DataFrame({'PointID': X_OOB['pointID'],
                                       'y_OOB': y_OOB,
                                       'y_validation_OOB': y_validation_OOB, }, )
        local_model_mse_OOB[SubSet.loc[SubSet['DNeighbour'] == 0, 'pointID'].values[0]] = mse_OOB
        local_model_mae_OOB[SubSet.loc[SubSet['DNeighbour'] == 0, 'pointID'].values[0]] = mae_OOB
        local_model_r2_OOB[SubSet.loc[SubSet['DNeighbour'] == 0, 'pointID'].values[0]] = r2_OOB
...
if Case == 'Classification':
    ...
    mean_l_mse = np.mean((aggregated_df['y_test_l'] - aggregated_df['y_pred_l'])**2)
    mean_l_mae = np.mean(np.abs(aggregated_df['y_test_l'] - aggregated_df['y_pred_l']))
    sst_l_r2 = np.sum((aggregated_df['y_test_l'] - np.mean(aggregated_df['y_test_l'])) ** 2)  # total sum of squares
    ssr_l_r2 = np.sum((aggregated_df['y_pred_l'] - aggregated_df['y_test_l']) ** 2)
    mean_l_r2 = 1 - (ssr_l_r2 / sst_l_r2)

    # Print or use the mean values as needed
    print("Mean MSE:", mean_l_mse)
    print("Mean MAE:", mean_l_mae)
    print("Mean R2:", mean_l_r2)

    print('############ Local model Metrics OOB VALIDATION #############')

    # Group by PointID and sum the values for each group
    aggregated_df_OOB = Validation_OOB_df.groupby('PointID').agg({
        'y_OOB': 'first',
        'y_validation_OOB': 'mean'
    }).reset_index()

    # Calculate the mean of each list of values
    mean_OOB_mse = np.mean((aggregated_df_OOB['y_OOB'] - aggregated_df_OOB['y_validation_OOB'])**2)
    mean_OOB_mae = np.mean(np.abs(aggregated_df_OOB['y_OOB'] - aggregated_df_OOB['y_validation_OOB']))
    sst_l_r2_OOB = np.sum((aggregated_df_OOB['y_OOB'] - np.mean(aggregated_df_OOB['y_OOB'])) ** 2)  # total sum of squares
    ssr_l_r2_OOB = np.sum((aggregated_df_OOB['y_validation_OOB'] - aggregated_df_OOB['y_OOB']) ** 2)
    mean_OOB_r2 = 1 - (ssr_l_r2_OOB / sst_l_r2_OOB)

    # Print or use the mean values as needed
    print("Mean MSE:", mean_OOB_mse)
    print("Mean MAE:", mean_OOB_mae)
    print("Mean R2:", mean_OOB_r2)

if exportLocalFeatureImportanceSHP:
    temp_df = df_local.drop(['pointID', 'z_score'], axis=1)
    ...
    if Case == 'Regression':
        localmse_df = pd.DataFrame.from_dict(local_model_mse_l, orient='index', columns=['MSE'])
        localmae_df = pd.DataFrame.from_dict(local_model_mae_l, orient='index', columns=['MSE'])
        localr2_df = pd.DataFrame.from_dict(local_model_r2_l, orient='index', columns=['MSE'])
        ExportDF = pd.concat([coords, Localfeature_df, localmse_df, localmae_df, localr2_df], axis=1)
...
X_evaluate = Vdf.drop(['z_score'], axis=1)
y_evaluate = Vdf['z_score']
...
if Case == 'Regression':
    # Calculate evaluation metrics
    mse = mean_squared_error(y_evaluate, Global_model_y_predict)
    mae = mean_absolute_error(y_evaluate, Global_model_y_predict)
    r2 = r2_score(y_evaluate, Global_model_y_predict)
...
if Case == 'Classification':
    ...
    prediction_df_final = pd.DataFrame(columns=['PointID', 'y_test_l', 'y_pred_l'])
...
if Case == 'Classification':
    ...
    LastDataframe = pd.concat([Vdf['z_score'],Global_model_y_predict.rename('Global_Model'),sorted_aggregated_df_final['Binary_Predictions'],sorted_aggregated_df_final['PointID']], axis=1)
    threshold = 0.5
    ...
    TP05 = ((LastDataframe['z_score'] == 1) & (LastDataframe['LinearCOmbination0.5'] == 1)).sum()
    TP025 = ((LastDataframe['z_score'] == 1) & (LastDataframe['LinearCOmbination0.25'] == 1)).sum()
    TP075 = ((LastDataframe['z_score'] == 1) & (LastDataframe['LinearCOmbination0.75'] == 1)).sum()

    # Calculate True Negatives (TN)
    TN05 = ((LastDataframe['z_score'] == 0) & (LastDataframe['LinearCOmbination0.5'] == 0)).sum()
    TN025 = ((LastDataframe['z_score'] == 0) & (LastDataframe['LinearCOmbination0.25'] == 0)).sum()
    TN075 = ((LastDataframe['z_score'] == 0) & (LastDataframe['LinearCOmbination0.75'] == 0)).sum()

    # Calculate False Positives (FP)
    FP05 = ((LastDataframe['z_score'] == 0) & (LastDataframe['LinearCOmbination0.5'] == 1)).sum()
    FP025 = ((LastDataframe['z_score'] == 0) & (LastDataframe['LinearCOmbination0.25'] == 1)).sum()
    FP075 = ((LastDataframe['z_score'] == 0) & (LastDataframe['LinearCOmbination0.75'] == 1)).sum()

    # Calculate False Negatives (FN)
    FN05 = ((LastDataframe['z_score'] == 1) & (LastDataframe['LinearCOmbination0.5'] == 0)).sum()
    FN025 = ((LastDataframe['z_score'] == 1) & (LastDataframe['LinearCOmbination0.25'] == 0)).sum()
    FN075 = ((LastDataframe['z_score'] == 1) & (LastDataframe['LinearCOmbination0.75'] == 0)).sum()
    ...
    # Calculate the mean of each list of values
    mean_final_mse = np.mean((aggregated_df_final['y_test_l'] - aggregated_df_final['y_pred_l']) ** 2)
    mean_final_mae = np.mean(np.abs(aggregated_df_final['y_test_l'] - aggregated_df_final['y_pred_l']))
    sst_final_r2 = np.sum((aggregated_df_final['y_test_l'] - np.mean(aggregated_df_final['y_test_l'])) ** 2)  # total sum of squares
    ssr_final_r2 = np.sum((aggregated_df_final['y_pred_l'] - aggregated_df_final['y_test_l']) ** 2)
    mean_final_r2 = 1 - (ssr_final_r2 / sst_final_r2)

    # Print or use the mean values as needed
    print("Predicted MSE:", mean_final_mse)
    print("Predicted MAE:", mean_final_mae)
    print("Predicted R2:", mean_final_r2)
    ...
    LastDataframe = pd.concat([Vdf['z_score'], Global_model_y_predict.rename('Global_Model'),
                               sorted_aggregated_df_final['y_pred_l'], sorted_aggregated_df_final['PointID']],
                              axis=1)

    # Mean Squared Error (MSE)
    mse_05 = np.mean((0.5* (LastDataframe['z_score'] - LastDataframe['Global_Model']) + 0.5*(
                LastDataframe['z_score']-LastDataframe['y_pred_l'])) ** 2)
    mse_25 = np.mean((0.25 * (LastDataframe['z_score'] - LastDataframe['Global_Model']) + 0.75 * (
                LastDataframe['z_score'] - LastDataframe['y_pred_l'])) ** 2)
    mse_75 = np.mean((0.75 * (LastDataframe['z_score'] - LastDataframe['Global_Model']) + 0.25 * (
                LastDataframe['z_score'] - LastDataframe['y_pred_l'])) ** 2)

    print('################ Linear Combintion Mean Squared Error')
    print(f'MSE for 0.5 weights for both Global and Local models = {mse_05}')
    print(f'MSE for 0.25 weight for GLobal and 0.75 weight for Locals = {mse_25}')
    print(f'MSE for 0.75 weight for GLobal and 0.25 weight for Locals = {mse_75}')

    # Mean Absolute Error (MSE)
    mae_05 = np.mean(np.abs(0.5 * (LastDataframe['z_score'] - LastDataframe['Global_Model']) + 0.5 * (
            LastDataframe['z_score'] - LastDataframe['y_pred_l'])))
    mae_25 = np.mean(np.abs(0.25 * (LastDataframe['z_score'] - LastDataframe['Global_Model']) + 0.75 * (
            LastDataframe['z_score'] - LastDataframe['y_pred_l'])))
    mae_75 = np.mean(np.abs(0.75 * (LastDataframe['z_score'] - LastDataframe['Global_Model']) + 0.25 * (
            LastDataframe['z_score'] - LastDataframe['y_pred_l'])))

    print('################ Linear Combintion Mean Absolute Error')
    print(f'MAE for 0.5 weights for both Global and Local models = {mae_05}')
    print(f'MAE for 0.25 weight for GLobal and 0.75 weight for Locals = {mae_25}')
    print(f'MAE for 0.75 weight for GLobal and 0.25 weight for Locals = {mae_75}')

    # R-squared (R2)
    sst_all = np.sum((LastDataframe['z_score'] - np.mean(LastDataframe['z_score'])) ** 2)  # total sum of squares
    ssr_05 = np.sum((0.5 * (LastDataframe['Global_Model'] - LastDataframe['z_score']) + 0.5 * (
            LastDataframe['y_pred_l'] - LastDataframe['z_score']))**2)
    ssr_25 = np.sum((0.25 * (LastDataframe['Global_Model'] - LastDataframe['z_score']) + 0.75 * (
            LastDataframe['y_pred_l'] - LastDataframe['z_score']))**2)  # regression sum of squares
    ssr_75 =np.sum((0.75 * (LastDataframe['Global_Model'] - LastDataframe['z_score']) + 0.25 * (
            LastDataframe['y_pred_l'] - LastDataframe['z_score']))**2) # regression sum of squares
    r2_05 = 1 - (ssr_05 / sst_all)
    r2_25 = 1 - (ssr_25 / sst_all)
    r2_75 = 1 - (ssr_75 / sst_all)

    print('################ Linear Combintion R squared')
    print(f'R2 for 0.5 weights for both Global and Local models = {r2_05 }')
    print(f'R2 for 0.25 weight for GLobal and 0.75 weight for Locals = {r2_25}')
    print(f'R2 for 0.75 weight for GLobal and 0.25 weight for Locals = {r2_75}')

if AnalyzeResiduales:
    print('########### Combination of linear global model with nonlinear local models ######### ')
    LastLinearDataframe = pd.concat([Vdf['z_score'],Linear_model_y_predict.rename('Global_Model'),sorted_aggregated_df_final['Binary_Predictions'],sorted_aggregated_df_final['NonLinear'],sorted_aggregated_df_final['PointID']], axis=1)
    threshold = 0.5
