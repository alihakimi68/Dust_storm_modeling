...
EmptyDf = pd.DataFrame(columns=['Soil_evaporation', 'Lakes', 'landcover', 'Precipitation', 'Soil_moisture',
                                ...
                                'Profile_curvature', 'Distance_to_river', 'Slope','Wind_Speed', 'dust_storm',
                                'Year','X','Y'])
...
Datatype = {'Soil_evaporation': 'numerical', 'Lakes': 'categorical', 'landcover': 'categorical',
            ...
            'Distance_to_river': 'numerical', 'Slope': 'numerical','Wind_Speed':'numerical', 'dust_storm': 'Label',
            'Year':'Year','X':'X','Y':'Y'}
...
def createDatasetFunc(year_list,window_size,dustsourcespickle):
    ...
    #         raster_paths = ["soil_evaporation/soil_evap_"+str(year)+".tif", "lakes/lakes_"+str(year),"land_use/landuse_"+str(year), "precipitation/prec_"+str(year), "soil_moisture/sm_"+str(year),"ndvi/ndvi_"+str(year), "elevation", "soil", "aspect", "curvature", "plan_c", "profile_c", "distance_riv", "slope","Wind_Speed/WS_"+str(year)+".tif"]
    #
    ...
    #
    #                         # Convert both the point and lake_coords to geographic coordinates
    #                         lake_coords_geo = [raster_transform * (coord[1] + 0.5, coord[0] + 0.5) for coord in
    #                                            lake_coords]
    #
    #                         # Calculate distances between the current point and all lake coordinates
    #                         distances_lakes = cdist([point], lake_coords_geo)
    #
    #                         # Find the index of the minimum distance
    #                         min_distance_index = np.argmin(distances_lakes)
    #
    #                         # Get the coordinates of the closest 1 value in the lake raster
    #                         closest_lake_coords = lake_coords_geo[min_distance_index]
    #                         lat_closest_lake, lon_closest_lake = closest_lake_coords
    #
    ...
    #
    #                         # Convert both the point and lake_coords to geographic coordinates
    #                         Cropland_coords_geo = [raster_transform * (coord[1] + 0.5, coord[0] + 0.5) for coord in
    #                                            Cropland_coords]
    #
    #                         # Calculate distances between the current point and all lake coordinates
    #                         Cropland_distances = cdist([point], Cropland_coords_geo)
    #
    #                         # Find the index of the minimum distance
    #                         min_distance_index_Cropland = np.argmin(Cropland_distances)
    #
    #                         # Get the coordinates of the closest 1 value in the lake raster
    #                         closest_coords_Cropland = Cropland_coords_geo[min_distance_index_Cropland]
    #                         lat_closest_Cropland, lon_closest_Cropland = closest_coords_Cropland
    #
    ...
    #
    #                         # Convert both the point and lake_coords to geographic coordinates
    #                         ndvi_coords_geo = [raster_transform * (coord[1] + 0.5, coord[0] + 0.5) for coord in
    #                                            valid_ndvi_coords]
    #
    #                         # Calculate distances between the current point and all valid NDVI coordinates
    #                         distances_ndvi = cdist([point], ndvi_coords_geo)
    #
    #                         # Find the index of the minimum distance
    #                         min_distance_index_ndvi = np.argmin(distances_ndvi)
    #
    #                         # Get the coordinates of the closest NDVI value in the specified range
    #                         closest_ndvi_coords = ndvi_coords_geo[min_distance_index_ndvi]
    #
    #                         lat_closest_ndvi, lon_closest_ndvi = closest_ndvi_coords
    #
    ...
    #
    #                         # Convert both the point and lake_coords to geographic coordinates
    #                         ndvi_dense_coords_geo = [raster_transform * (coord[1] + 0.5, coord[0] + 0.5) for coord in
    #                                            valid_ndvi_coords_dense]
    #
    #                         # Calculate distances between the current point and all valid NDVI coordinates
    #                         distances_ndvi_dense = cdist([point], ndvi_dense_coords_geo)
    #
    #                         # Find the index of the minimum distance
    #                         min_distance_index_ndvi_dense = np.argmin(distances_ndvi_dense)
    #
    #                         # Get the coordinates of the closest NDVI value in the specified range
    #                         closest_ndvi_coords_dense = ndvi_dense_coords_geo[min_distance_index_ndvi_dense]
    #
    #                         lat_closest_ndvi_dense, lon_closest_ndvi_dense = closest_ndvi_coords_dense
    #
    #                         # Calculate distance using Haversine formula
    ...
    #
    #                     # if "Wind_Speed/WS" in raster_path:
    #                     #     raster_transform = raster.transform
    #                     #     # If the current raster is the lakes raster, find the closest value of 1
    #                     #     # Load the lake raster as a numpy array
    #                     #     WS_data = raster.read(1)
    #                     #
    #                     #     # Find the indices of WS values bigger the [3.2]
    #                     #     valid_WS_indices = np.where((WS_data >= 3.2) )
    #                     #
    #                     #     # Create a list of coordinates within the specified NDVI range
    #                     #     valid_WS_coords = list(zip(valid_WS_indices[0], valid_WS_indices[1]))
    #                     #
    #                     #     # Convert both the point and lake_coords to geographic coordinates
    #                     #     valid_WS_coords_geo = [raster_transform * (coord[1] + 0.5, coord[0] + 0.5) for coord in
    #                     #                              valid_WS_coords]
    #                     #
    #                     #     # Calculate distances between the current point and all lake coordinates
    #                     #     WS_distances = cdist([point], valid_WS_coords_geo)
    #                     #
    #                     #     # Find the index of the minimum distance
    #                     #     min_distance_index_WS = np.argmin(WS_distances)
    #                     #
    #                     #     # Get the coordinates of the closest 1 value in the lake raster
    #                     #     closest_coords_WS = valid_WS_coords_geo[min_distance_index_WS]
    #                     #
    #                     #     lat_closest_WS, lon_closest_WS = closest_coords_WS
    #                     #
    #                     #     distance_km_WS = haversine(lat_dust_source, lon_dust_source, lat_closest_WS,
    #                     #                                      lon_closest_WS)
    #                     #     values.append(distance_km_WS)
    #
    #
    ...
    if window_size == 0:
        ...
        #for year in range(2001):
            ...
            raster_paths = ["soil_evaporation/soil_evap_"+str(year)+".tif",
                            ...
                            "plan_c","profile_c","distance_riv","slope","Wind_Speed/WS_"+str(year)+".tif"]
    ...
    if CalculateSeasons:
        ...
        for period in periods:
            ...
            if CalculateDistance:
                DistStr = 'Dist'
            WS = 'WS'
            dustsourcespickle = f'df_dustsources_WS{window_size}_X_{window_size}_PN{PeriodName}_SP_{statisticalParams}_{DistStr}_{WS}'
